...
# RBM training
def train_rbm(data, nv, nh, lr=0.1, epochs=5000, k=100):
    W = np.random.randn(nv, nh) * 0.1
    b = np.zeros(nv)
    c = np.zeros(nh)
...
# RBM sampling
def sample_rbm(W, b, c, n=10000, burn=2000):
    v = samp_pm(np.full((1, W.shape[0]), 0.5))
    counts = {}
...
Ms = [1, 2, 4, 8]
kl_best = [np.inf]*len(Ms)
best_models = [None]*len(Ms)

# Train RBM multiple times and pick the best model per hidden size
for it in range(10):
    for M_idx, M in enumerate(Ms):
        W, b, c = train_rbm(data, nv=3, nh=M, lr=0.1, epochs=5000, k=100)
        q = sample_rbm(W, b, c, n=10000)
        current_kl = KL(p, q)
        if current_kl < kl_best[M_idx]:
            kl_best[M_idx] = current_kl
            best_models[M_idx] = (W, b, c)
    print(f"Iteration {it+1}, best KL so far: {kl_best}")
